...
const registerUser = async (request, response, next) => {
    ...
    try {
        ...
        bcrypt.hash(password, saltRounds, async (err, hashedPassword) => {
            if (err)
                throwError("Error occurred hashing password", 500, next);
            // construct payload
...
const loginUser = async (request, response, next) => {
    ...
    try {
        ...
        const storedHashPassword = userInfo?.password;
...
const forgotPassword = async (request, response, next) => {
    ...
    try {
        ...
        bcrypt.hash(password, saltRounds, async (err, hashedPassword) => {
            if (err)
                return throwError("Error occurred hashing password", 403, next);
            // if hash success
            const newPayload = {
            ...
            const updatePassword = await Register.findOneAndUpdate({ email }, { password: hashedPassword });
            ...
            if (updatePassword) {
                ...
                try {
                    ...
                    return throwError("Failed to send email as password updated successfully", 500, next);
                }
                return response.status(200).json({
                    success: true,
...
const checkAuthenticateUser = async (request, response, next) => {
    ...
    if (decodeToken && decodeToken?.userId && decodeToken?.email) {
        ...
        try {
            ...
            return throwError("Error occurred confirming user from database", 500, next);
        }
    }
    else {
...
export { registerUser, loginUser, logoutUser, forgotPassword, checkAuthenticateUser, };
