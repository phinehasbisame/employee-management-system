...
const getAllUser = async (
  ...
  try {
    ...
    const page = (request.query.page as unknown as number) * 1 || 1;
    const pageSize = (request.query.pageSize as unknown as number) * 1 || 5;
...
const updateUser = async (
  ...
  try {
    const updateUser = await User.findByIdAndUpdate(id, updateDetails).select(
      "-__v",
    );
    if (!updateUser) {
